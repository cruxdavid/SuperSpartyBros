...
public class CharacterController2D : MonoBehaviour
{
	...
	void Update ()
	{
		...
		if (isGrounded && CrossPlatformInputManager.GetButtonDown ("Jump")) { // If grounded AND jump button pressed, then allow the player to jump
			DoJump ();	
		} else if (canDoubleJump && CrossPlatformInputManager.GetButtonDown ("Jump")) { // If canDoubleJump = T and jump button pressed, then allow player to double jump
			DoJump();
			canDoubleJump = false; //Disable double jump after double jumping, since you can only do it once.
		}
		...
		Physics2D.IgnoreLayerCollision (_playerLayer, _groundLayer, (_vy > 0.0f));
	}
